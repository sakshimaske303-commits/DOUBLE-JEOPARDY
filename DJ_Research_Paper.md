...
Coral degradation manifests primarily as thermally driven bleaching rather than area loss, so I measured condition using the continuous Degree Heating Week time series rather than mapped physical extent — comparing, for each island, an early reference period (1996–2000) against a recent one (2016–2020). Each island's DHW series is queried from a single representative coordinate near its reef area, not a spatial average across the full reef extent — a coarser sampling choice than area-weighted aggregation would give (see Section 6).
