...
|---|---|---|
| SLR Exposure (5 islands) | Interactive (Canary: Python/folium; Maldives, Seychelles, Fiji, Lakshadweep: QGIS2Web) | Complete, zoom issue resolved |
| Ecosystem Buffer Overview (5 islands) | Interactive (Python/folium) | Complete, dynamic per-island legend |
...
- Compound Vulnerability Score: only the normalization range changes (Canary Islands itself stays the fixed 0.000 floor, since it was already the minimum). Seychelles 0.880 → 0.895, Lakshadweep 0.467 → 0.481, Fiji 0.217 → 0.263, Maldives and Canary Islands unchanged. **Ranking is identical** — Seychelles still highest, Canary Islands still lowest.
- Weighting-sensitivity crossover point (Figure 7): 74.5% → 76.8% physical-exposure weighting. Still well clear of the 50/50 weighting actually used.
- Governance-alignment correlation: r=0.727 → r=0.718, p=0.164 → p=0.172, 95% CI [-0.43, 0.98] → [-0.45, 0.98]. Same interpretation (suggestive, not significant).

Fixed `slr_exposure_analysis.py` to apply the Canary-specific NoData exclusion directly (documented inline, with the reasoning above), rather than leaving the correction living only in a separate elevation-export script. Updated `data/slr_exposure_summary.csv`, `data/compound_vulnerability_scores.csv`, `data/governance_alignment_test.csv`, `compound_vulnerability_score.py`, `governance_correlation_test.py`, `research_paper_figures.py`, `map1_vulnerability_score.py`, and `verify_normalization.py` to match, and regenerated every figure that depends on these numbers (`fig2`, `fig3`, `fig6`, `fig7`, `compound_vulnerability_score.png`). Updated `DJ_Research_Paper.md`, `DJ_Project_Report.md`, `README.md`, and dashboard pages 3, 4, 5, and the home page — including two places where the dashboard's *static* cards were already quietly showing 12.1% right next to a *live* slider on the same page that, because it read from the already-corrected `settlement_elevations.csv`, was already showing 0.3% — an internal inconsistency on the live site that this pass also resolves.
...
**Attempt 1** pulled a single Sentinel-2 scene per year via the Process API, picking "least cloudy" by comparing `dataMask` values — which is binary (0/1), not a cloud percentage, so it didn't actually filter clouds at all. Result: 8,225 km² of "new built-up area" for Maldives, a country whose entire land area is ~298 km². Caught immediately on scale alone.

**Attempt 2** switched to real per-pixel cloud masking via the Sentinel-2 Scene Classification Layer (SCL) and averaged NDBI over every cloud-free scene in the year rather than picking one. Better, but still tried to classify water vs. land scene-by-scene using SCL, and that classification wasn't even consistent between 2016 and 2024 for the same physical location — creating spurious "change" at every coastline. Also hit a transient issue where a failed request for one year silently left the *previous* run's file in place, so a stale, already-known-bad diff raster kept getting reused without either of us noticing at first — fixed by having the script delete a stale diff file outright when it can't regenerate both years in the same run, rather than leaving it for a later step to accidentally trust.

**Attempt 3** replaced the per-scene water/land guess with a fixed mask: this project's own already-validated island boundary polygons (`data/boundaries/*.gpkg`, the same ones `population_weighted_exposure.py` uses), rasterized onto the NDBI grid and applied identically to both years. This fixed the coastline-noise problem, but a new pattern showed up: all three islands — geologically nothing alike (coral atoll, granite islands, volcanic) — landed within a narrow 42–55% band of "land area exceeding a fixed ±0.05 NDBI change threshold," split close to evenly between increase and decrease. A genuine 8-year urbanization signal should skew toward increase; a near-50/50 split pointed to threshold noise, not a remaining pipeline bug. (Also checked whether the January 2022 Sentinel-2 processing-baseline radiometric offset — a known real issue when comparing pre/post-2022 scenes — was responsible; Copernicus's own documentation confirms `harmonizeValues` defaults to true and handles this automatically, so that wasn't it.)
...
**Ecosystem Buffer Overview — done for all five islands.** `build_ecosystem_buffer_map.py` reads the same source layers the QGIS project already used (`data/ecosystem_buffers/*_mangroves_clean.gpkg`, `*_coral_clean.gpkg`, `*_wdpa.gpkg`, `data/boundaries/*_islands.gpkg`) and reproduces the same per-island layer set the dashboard already documents — Lakshadweep coral + boundary only, Canary WDPA + boundary only, the other three all four layers. First pass rendering each feature as its own `folium.GeoJson` call blew Canary's file up to 86MB (385 WDPA polygons at full source vertex density — 1.8M points total — plus per-feature popup wrapper overhead multiplies fast in Leaflet's generated JS). Fixed two ways: combined each layer into one `folium.GeoJson` call instead of one per feature, and simplified geometry before rendering (0.0005° tolerance for WDPA, 0.0001° for coral/mangroves — well under anything visible at island scale, doesn't touch the underlying feature count or attributes). Canary dropped to 2.4MB; every island's map is now under 3.5MB. Feature counts on every layer match the dashboard's existing per-island text exactly (Maldives 93 WDPA, Fiji 126, etc.) — confirms nothing about the actual data changed, just how it's rendered.

**SLR Exposure — Canary Islands done, the other four blocked on elevation data access.** Same settlement-elevation logic as `slr_exposure_analysis.py` (1m threshold, Canary's exact-zero-elevation NoData exclusion applied only to Canary), but Canary's map could be built immediately because `data/canary_python_elevations_check.csv` already had full per-point lon/lat/elevation sitting from the earlier QGIS-cross-check work — just needed to join it back to `canary_settlements_clean.gpkg` for names (verified row-for-row positional match on coordinates first) and rebuild in folium. 4,834 settlements, 13 at risk, 0.3% — exact match to the validated summary. The other four islands' elevation rasters (`data/terrain/{island}_elevation.tif`) are 260MB–1.5GB each and live only on the original machine, too large to move for a supplementary visualization — `export_settlement_elevations_full.py` is written and ready (same sampling logic, same Canary-only NoData rule), just needs one local run to produce the four missing per-island CSVs before `build_slr_exposure_map.py` can pick them up. Left the four QGIS2Web SLR maps live and linked in the meantime rather than breaking them to make a point about migration progress — the dashboard's SLR section legend and this file's status table both say plainly which four are still QGIS2Web so nothing here is overstated.
